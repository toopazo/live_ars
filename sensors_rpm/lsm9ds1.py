...
class LSM9DS1():
    def __init__(self):
        ...
        time.sleep(0.1)
        # Check ID registers.
        if self._read_byte(_XGTYPE,
                           _LSM9DS1_REGISTER_WHO_AM_I_XG) != _LSM9DS1_XG_ID or \
                self._read_byte(_MAGTYPE,
                                _LSM9DS1_REGISTER_WHO_AM_I_M) != _LSM9DS1_MAG_ID:
            raise RuntimeError('Could not find LSM9DS1, check wiring!')

        # enable gyro continuous
        self._write_byte(_XGTYPE, _LSM9DS1_REGISTER_CTRL_REG1_G,
                         [0xC0])  # on XYZ
    ...
    def read_accel_raw(self):
        ...
        raw_list = self._read_bytes(_XGTYPE,
                                    0x80 | _LSM9DS1_REGISTER_OUT_X_L_XL, 6)
    ...
    def acceleration(self):
        ...
        return list(map(lambda
                            x: x * self._accel_mg_lsb / 1000.0 * _SENSORS_GRAVITY_STANDARD,
                        raw))
    ...
    def read_mag_raw(self):
        ...
        raw_list = self._read_bytes(_MAGTYPE,
                                    0x80 | _LSM9DS1_REGISTER_OUT_X_L_M, 6)
        raw_x, raw_y, raw_z = struct.unpack_from('<hhh', bytes(raw_list))
        return (raw_x, raw_y, raw_z)
    ...
    def magnetic(self):
        ...
        magneticVal = list(
            map(lambda x: x * self._mag_mgauss_lsb / 1000.0, raw))

        if (self.magCalibrationCenter is not None) and (
                self.magCalibrationScale is not None):
    ...
    def read_gyro_raw(self):
        ...
        """
        # Read the gyroscope
        raw_list = self._read_bytes(_XGTYPE, 0x80 | _LSM9DS1_REGISTER_OUT_X_L_G,
                                    6)
        raw_x, raw_y, raw_z = struct.unpack_from('<hhh', bytes(raw_list))
        return (raw_x, raw_y, raw_z)
